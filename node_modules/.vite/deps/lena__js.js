...
// node_modules/lena.js/dist/index.js
var require_dist = __commonJS({
  "node_modules/lena.js/dist/index.js"(exports) {
    ...
    var contrast = function(pixels, amount) {
    ...
    var brightness = function(pixels, amount = 0) {
      let { data } = pixels;
    ...
    var gamma = function(pixels) {
      ...
      for (let i2 = 0; i2 < pixels.data.length; i2 += 4) {
        pixels.data[i2] = table[pixels.data[i2]];
        pixels.data[i2 + 1] = table[pixels.data[i2 + 1]];
        pixels.data[i2 + 2] = table[pixels.data[i2 + 2]];
      }
      return pixels;
    };
    ...
    var saturation = function(pixels) {
      const level = 2.9, RW = 0.3086, RG = 0.6084, RB = 0.082, RW0 = (1 - level) * RW + level, RW1 = (1 - level) * RW, RW2 = (1 - level) * RW, RG0 = (1 - level) * RG, RG1 = (1 - level) * RG + level, RG2 = (1 - level) * RG, RB0 = (1 - level) * RB, RB1 = (1 - level) * RB, RB2 = (1 - level) * RB + level;
      for (let i = 0; i < pixels.data.length; i += 4) {
    ...
    var thresholding = function(pixels, amount = 128) {
      for (let i = 0; i < pixels.data.length; i += 4) {
        let r = pixels.data[i], g = pixels.data[i + 1], b = pixels.data[i + 2];
    ...
    var noise = function(pixels, amount = 0) {
    ...
    var convolution = function(pixels, kernel) {
      ...
      for (let y = 0; y < canvasHeight; y++) {
        for (let x = 0; x < canvasWidth; x++) {
          let dstOff = (y * canvasWidth + x) * 4, sumReds = 0, sumGreens = 0, sumBlues = 0;
          for (let kernelY = 0; kernelY < side; kernelY++) {
            for (let kernelX = 0; kernelX < side; kernelX++) {
              let currentKernelY = y + kernelY - halfSide, currentKernelX = x + kernelX - halfSide;
              if (currentKernelY >= 0 && currentKernelY < canvasHeight && currentKernelX >= 0 && currentKernelX < canvasWidth) {
                let offset = (currentKernelY * canvasWidth + currentKernelX) * 4, weight = kernel[kernelY * side + kernelX];
    ...
    var bigGaussian = function(pixels) {
      const divider = 159;
      const operator = [
        2 / divider,
        4 / divider,
    ...
    var gradient = function(deltaX, deltaY) {
      let srcX = deltaX.data, canvasWidth = deltaX.width, canvasHeight = deltaX.height, srcY = deltaY.data, temporaryCanvas = document.createElement("canvas"), temporaryCtx = temporaryCanvas.getContext("2d"), outputData = temporaryCtx.createImageData(canvasWidth, canvasHeight), outputDataDir = new Array(srcX.length).fill(0);
      for (let y = 0; y < canvasHeight; y++) {
        for (let x = 0; x < canvasWidth; x++) {
    ...
    var nonMaximumSuppression = function(pixels, direction) {
      let side = Math.round(Math.sqrt(25)), halfSide = Math.floor(side / 2), src = pixels.data, canvasWidth = pixels.width, canvasHeight = pixels.height, temporaryCanvas = document.createElement("canvas"), temporaryCtx = temporaryCanvas.getContext("2d"), outputData = temporaryCtx.createImageData(canvasWidth, canvasHeight);
      for (let y = 0; y < canvasHeight; y++) {
        for (let x = 0; x < canvasWidth; x++) {
          let dstOff = (y * canvasWidth + x) * 4, maxReds = src[dstOff], maxGreens = src[dstOff + 1], maxBlues = src[dstOff + 2];
          for (let kernelY = 0; kernelY < side; kernelY++) {
            for (let kernelX = 0; kernelX < side; kernelX++) {
              let currentKernelY = y + kernelY - halfSide, currentKernelX = x + kernelX - halfSide;
              if (currentKernelY >= 0 && currentKernelY < canvasHeight && currentKernelX >= 0 && currentKernelX < canvasWidth) {
                let offset = (currentKernelY * canvasWidth + currentKernelX) * 4, currentKernelAngle = Math.atan2(currentKernelY - y, currentKernelX - x);
    ...
    var filterImage = function(selector, filter, image, amount = 0) {
      let args = [getImage(image), amount];
      return printCanvas(selector, filter.apply(null, args));
    };
    var redrawCanvas = function(selector, filter) {
    ...
    var histogram = function(image) {
      const pixels = this.getImage(image), zeroedArray = new Array(257).join("0").split("");
      let histogramTemp = {
...
//# sourceMappingURL=lena__js.js.map
